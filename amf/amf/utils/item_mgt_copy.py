...
DEFAULT_BATCH_VALUATION_BASE_YEAR = 2020
...
DEFAULT_BATCH_VALUATION_TARGETS = (
    ...
    {"code_prefix": "21", "code_length": 6, "target_rate": 20.5},
    {
        "code_prefix": "2",
        "code_length": 6,
        "target_rate": 20.0,
    ...
    {"code_prefix": "30", "code_length": 6, "target_rate": 21.25},
)
...
def enqueue_configured_scrap_batch_valuation_reconciliation(
    ...
    _enqueue_long_job(
        ...
        receipt_date_from=receipt_date_from,
        receipt_date_to=receipt_date_to,
        export_audit_records=cint(export_audit_records),
    )
    return None
...
def reconcile_batch_valuation_rate(
    ...
    receipt_date_from, receipt_date_to = _normalize_receipt_window(
...
def reconcile_batch_valuation_rate_by_code_prefix(
    ...
    exclude_prefixes = _normalize_code_prefix_exclusions(exclude_prefixes)
    if not code_prefix:
    ...
    return _run_batch_valuation_reconciliation(
        ...
        warehouse=warehouse,
        target_rate=target_rate,
        scope_label=_format_code_rule_label(
            code_prefix, code_length, exclude_prefixes=exclude_prefixes
        ),
        scope_summary={
            "code_prefix": code_prefix,
            "code_length": code_length,
...
def reconcile_configured_scrap_batch_valuation_reconciliation(
    ...
    """
    Run the configured code-prefix reconciliation rules one period at a time.

    By default the method produces one separate summary per configured receipt
    period, starting at the posting year and then continuing through 2020 and
    any earlier years that actually exist in Scrap history.

    If `receipt_date_from` and `receipt_date_to` are passed explicitly, only
    that one period is processed. The return payload aggregates the individual
    summaries so production runs can be audited from a single response.
    """
    dry_run = cint(dry_run)
    export_audit_records = cint(export_audit_records)
    ...
    receipt_periods = _get_configured_receipt_periods(warehouse)
    if receipt_date_from and receipt_date_to:
        receipt_periods = (
            {
                "label": "received between {0} and {1}".format(
                    receipt_date_from, receipt_date_to
                ),
                "receipt_date_from": receipt_date_from,
                "receipt_date_to": receipt_date_to,
    ...
    for receipt_period in receipt_periods:
        period_summary = _run_configured_scrap_batch_valuation_reconciliation_for_period(
            warehouse=warehouse,
            receipt_date_from=receipt_period["receipt_date_from"],
            receipt_date_to=receipt_period["receipt_date_to"],
            period_label=receipt_period["label"],
            dry_run=dry_run,
            max_rows_per_reconciliation=max_rows_per_reconciliation,
        )
        summary["period_summaries"].append(period_summary)
        summary["group_summaries"].extend(period_summary.get("group_summaries", []))
        summary["reconciliations"].extend(period_summary.get("reconciliations", []))
        summary["difference_amount"] += flt(period_summary.get("difference_amount"))
        summary["items_count"] += cint(period_summary.get("items_count"))
        summary["batch_count"] += cint(period_summary.get("batch_count"))

    if export_audit_records:
        summary["audit_export"] = _export_scrap_batch_valuation_audit_records(summary)
...
def _get_configured_receipt_periods(warehouse):
    """
    Build the default yearly receipt windows for Scrap valuation reconciliation.

    The generated periods always include the posting year down to 2020, and
    extend further back automatically if older Scrap receipts exist.
    """
    posting_date, posting_time = _get_posting_context()
    posting_year = getdate(posting_date).year
    earliest_year = _get_earliest_scrap_receipt_year(
        warehouse=warehouse,
        posting_date=posting_date,
        posting_time=posting_time,
    )
    start_year = min(DEFAULT_BATCH_VALUATION_BASE_YEAR, earliest_year or posting_year)

    periods = []
    for year in range(posting_year, start_year - 1, -1):
        periods.append(
            {
                "label": "received in {0}".format(year),
                "receipt_date_from": "{0}-01-01".format(year),
                "receipt_date_to": "{0}-12-31".format(year),
            }
        )
...
def _get_earliest_scrap_receipt_year(warehouse, posting_date, posting_time):
    """
    Return the first year with a positive Scrap receipt before the posting cutoff.

    This allows the configured yearly summaries to expand automatically when
    the historical database contains older relevant batch receipts.
    """
    earliest_posting_date = frappe.db.sql(
        """
    ...
    if not earliest_posting_date:
        return None

    return getdate(earliest_posting_date).year


def _export_scrap_batch_valuation_audit_records(summary):
    """
    Write a compact audit export for one configured Scrap reconciliation run.

    Two files are generated:
    1. CSV with one row per year/prefix rule
    2. JSON with the full returned summary payload
    """
    timestamp = now_datetime().strftime("%Y%m%d_%H%M%S")
    mode_label = "dry_run" if summary.get("dry_run") else "live"
    file_stub = "scrap_batch_valuation_audit_{0}_{1}".format(mode_label, timestamp)
    csv_path = frappe.get_site_path("private", "files", "{0}.csv".format(file_stub))
    json_path = frappe.get_site_path("private", "files", "{0}.json".format(file_stub))
    audit_rows = _build_scrap_batch_valuation_audit_rows(summary)

    with open(csv_path, "w", newline="") as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=list(audit_rows[0].keys()))
        writer.writeheader()
    ...
    return {
        "csv_path": csv_path,
        "json_path": json_path,
        "rows_count": len(audit_rows),
    }


def _build_scrap_batch_valuation_audit_rows(summary):
    """
    Flatten the nested Scrap valuation summary into one audit row per rule.

    Each row captures one year and one code-prefix rule so the CSV stays easy
    to filter and reconcile in external audit workpapers.
    """
    audit_rows = []

    for period_summary in summary.get("period_summaries", []):
        for group_summary in period_summary.get("group_summaries", []):
            exclude_prefixes = ", ".join(group_summary.get("exclude_prefixes") or [])
            audit_rows.append(
                {
                    "warehouse": summary.get("warehouse"),
                    "period_label": period_summary.get("period_label"),
                    "receipt_date_from": period_summary.get("receipt_date_from"),
                    "receipt_date_to": period_summary.get("receipt_date_to"),
                    "code_rule": _format_code_rule_label(
                        group_summary.get("code_prefix"),
                        cint(group_summary.get("code_length")),
                        exclude_prefixes=group_summary.get("exclude_prefixes"),
                    ),
                    "code_prefix": group_summary.get("code_prefix"),
                    "exclude_prefixes": exclude_prefixes,
                    "target_rate": group_summary.get("target_rate"),
                    "items_count": group_summary.get("items_count"),
                    "batch_count": group_summary.get("batch_count"),
                    "skipped_batches_already_at_target": group_summary.get(
                        "skipped_batches_already_at_target"
                    ),
                    "difference_amount": group_summary.get("difference_amount"),
                    "posting_date": group_summary.get("posting_date"),
                    "posting_time": group_summary.get("posting_time"),
                    "dry_run": group_summary.get("dry_run"),
                    "reconciliations_count": len(group_summary.get("reconciliations") or []),
                    "reconciliations": ", ".join(
                        group_summary.get("reconciliations") or []
                    ),
                    "log_id": group_summary.get("log_id"),
                }
            )
    ...
    return [
        {
            ...
            "items_count": 0,
            "batch_count": 0,
            "skipped_batches_already_at_target": 0,
            "difference_amount": 0.0,
            "posting_date": "",
            "posting_time": "",
...
def _run_configured_scrap_batch_valuation_reconciliation_for_period(
    warehouse,
    receipt_date_from,
    receipt_date_to,
    period_label,
    ...
    """
    Run the configured code-prefix rules for one historical Scrap receipt window.

    Batches are classified by the timestamp of their first positive receipt into
    the Scrap warehouse so one batch belongs to only one reporting period.
    """
    period_summary = {
    ...
    for rule in DEFAULT_BATCH_VALUATION_TARGETS:
        ...
        period_summary["group_summaries"].append(group_summary)
        period_summary["reconciliations"].extend(group_summary.get("reconciliations", []))
        period_summary["difference_amount"] += flt(group_summary.get("difference_amount"))
        period_summary["items_count"] += cint(group_summary.get("items_count"))
        period_summary["batch_count"] += cint(group_summary.get("batch_count"))
...
def _normalize_receipt_window(receipt_date_from=None, receipt_date_to=None):
    """
    Validate an optional Scrap receipt window and return ISO date strings.

    Both boundary dates must be provided together. The reconciliation query
    uses these dates to keep only batches whose first positive Scrap receipt
    falls inside the requested window.
    """
    if not receipt_date_from and not receipt_date_to:
        return None, None

    if not receipt_date_from or not receipt_date_to:
        frappe.throw("Both receipt_date_from and receipt_date_to are required.")

    receipt_date_from = getdate(receipt_date_from)
    receipt_date_to = getdate(receipt_date_to)

    if receipt_date_from > receipt_date_to:
        frappe.throw("receipt_date_from cannot be after receipt_date_to.")

    return str(receipt_date_from), str(receipt_date_to)


def _normalize_code_prefix_exclusions(exclude_prefixes=None):
    """
    Normalize optional code-prefix exclusions to a clean tuple of strings.

    The configured valuation rules use this to let a broad prefix such as `2`
    exclude a more specific prefix such as `21`, which avoids reconciling the
    same item code under two different target rates.
    """
    ...
    for prefix in exclude_prefixes:
        ...
        if prefix and prefix not in normalized_prefixes:
...
def _run_batch_valuation_reconciliation(
    ...
    max_rows_per_reconciliation = cint(max_rows_per_reconciliation)
    posting_date, posting_time = _get_posting_context()
    receipt_date_from = scope_summary.get("receipt_date_from")
    receipt_date_to = scope_summary.get("receipt_date_to")
    difference_amount = _get_total_difference_amount(batch_rows, target_rate)

    if receipt_date_from and receipt_date_to:
        scope_label = "{0} / first Scrap receipt between {1} and {2}".format(
            scope_label, receipt_date_from, receipt_date_to
        )
    ...
    update_log_entry(
        log_id,
        (
            "[{0}] Prepared {1} batch row(s) across {2} item(s) in {3} reconciliation document(s). Total difference amount={4}.<br>"
        ).format(
            now_datetime(),
            len(batch_rows),
            summary["items_count"],
...
def _create_reconciliation_log(
    ...
    reference_name = _build_reconciliation_log_reference(warehouse, scope_label)
    log_context = frappe._dict(
        {
...
def _build_reconciliation_log_reference(warehouse, scope_label):
    """
    Build a Log Entry reference name that stays within Frappe's field limit.

    The detailed scope remains in the log body; the reference name only needs
    to be unique and recognizable enough to find the run later.
    """
    reference_name = "Batch valuation {0} / {1} / {2}".format(
        warehouse,
        scope_label,
        now_datetime().strftime("%Y-%m-%d %H:%M:%S"),
    )
    max_reference_length = 140 - len("Stock Reconciliation: ")
    if len(reference_name) > max_reference_length:
        return "{0}...".format(reference_name[: max_reference_length - 3])
...
def _get_batch_rows_by_item_group(
    warehouse,
    item_group,
    target_rate,
    receipt_date_from=None,
...
def _get_batch_rows_by_code_prefix(
    warehouse,
    code_prefix,
    code_length,
    target_rate,
    ...
    for index, exclude_prefix in enumerate(exclude_prefixes):
        param_name = "exclude_like_pattern_{0}".format(index)
        extra_conditions += "\n          AND sle.item_code NOT LIKE %({0})s".format(
            param_name
        )
        params[param_name] = "{0}%".format(exclude_prefix)
...
def _fetch_batch_rows(
    ...
    When a receipt window is provided, a batch is kept only if its first
    positive receipt into the Scrap warehouse falls within that date range.
    """
    posting_date, posting_time = _get_posting_context()
    ...
    if receipt_date_from and receipt_date_to:
        ...
        receipt_window_having = """
          AND MIN(
                CASE
                    WHEN sle.actual_qty > 0 THEN TIMESTAMP(sle.posting_date, sle.posting_time)
                    ELSE NULL
                END
              ) >= TIMESTAMP(%(receipt_date_from)s, '00:00:00')
          AND MIN(
                CASE
                    WHEN sle.actual_qty > 0 THEN TIMESTAMP(sle.posting_date, sle.posting_time)
                    ELSE NULL
                END
              ) <= TIMESTAMP(%(receipt_date_to)s, '23:59:59')
        """
    ...
    batch_rows = frappe.db.sql(
        ...
        """.format(
            ...
            receipt_window_having=receipt_window_having,
        ),
        query_params,
        as_dict=True,
    )
...
def _get_total_difference_amount(batch_rows, target_rate):
    """
    Return the total value adjustment required to move all rows to target_rate.

    The amount is computed before document creation so dry runs can report the
    expected accounting impact without posting Stock Reconciliation documents.
    """
    difference_amount = 0.0

    for row in batch_rows:
        difference_amount += flt(row.qty) * (
            flt(target_rate) - flt(row.current_valuation_rate)
        )
...
def _format_code_rule_label(code_prefix, code_length, exclude_prefixes=None):
    """
    Render a readable log label for one code-prefix valuation rule.

    Example:
    `code-prefix 2xxxxx excluding 21xxxx`
    """
    exclude_prefixes = _normalize_code_prefix_exclusions(exclude_prefixes)
    label = "code-prefix {0}{1}".format(
    ...
    excluded_labels = [
        "{0}{1}".format(prefix, "x" * max(code_length - len(prefix), 0))
        for prefix in exclude_prefixes
    ]
    return "{0} excluding {1}".format(label, ", ".join(excluded_labels))
