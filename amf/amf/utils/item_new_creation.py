...
# Motor is always item code “591000” for all “49xxxx” items
MOTOR_ITEM_CODE = "591000"

# Mapping: third digit of finished-item_code → syringe item_name
SYRINGE_MAP = {
    "2": "S-50-P",
    "3": "S-100-P",
    "5": "S-250-P",
...
# Regular expression to filter exactly six‐digit codes that start with “49”
ITEM_CODE_REGEX = r"^49[0-9]{4}$"
LIKE_PATTERN_49_6 = "49____"

# ─────────────────────────────────────────────────────────────────────────────
# 1) QUERY ALL “49xxxx” ITEMS
# ─────────────────────────────────────────────────────────────────────────────

def get_finished_items():
    """
    Return a list of dicts for all Item documents whose item_code is exactly 6
    digits and starts with “49”. Each dict has: { item_code, item_name, stock_uom }.
    """
    candidates = frappe.get_all(
        "Item",
        filters=[["item_code", "like", LIKE_PATTERN_49_6]],
        fields=["item_code", "item_name", "stock_uom"]
    )

    # 2) In Python: filter so that len == 6 and third char in SYRINGE_MAP
    finished = []
    for itm in candidates:
        code = itm["item_code"]
        if len(code) == 6 and code[0:2] == "49" and code[2] in SYRINGE_MAP:
            finished.append(itm)
        # else: skip any that don’t have a valid third character or wrong length
    return finished

# ─────────────────────────────────────────────────────────────────────────────
# 2) HELPER TO EXTRACT VALVE-HEAD NAME FROM item_name
# ─────────────────────────────────────────────────────────────────────────────

def extract_valve_head_name(item_name):
    """
    Given an item_name string, extract the substring between the first two '/' characters.
    e.g. "ABC/ValveHeadName/XYZ" → "ValveHeadName".
...
# ─────────────────────────────────────────────────────────────────────────────
# 3) BULK-FETCH ALL COMPONENT INFO (motor, syringes, valve heads)
# ─────────────────────────────────────────────────────────────────────────────

def fetch_all_component_info(finished_items):
    """
    Given the list of finished_items (each has item_code, item_name, stock_uom),
    this function collects:
      • all distinct valve_head names (from between slashes in item_name)
      • all syringe item_names (from SYRINGE_MAP values)
      • the single motor item_code = “591000”

    Then does as few DB calls as possible to build a lookup dict:

        {
            "591000":      { "item_code": "591000",    "uom": "...motor_uom..." },
            "S-50-P":      { "item_code": "XXXXX",     "uom": "...uom..."       },
            "ValveHeadA":  { "item_code": "YYYYY",     "uom": "...uom..."       },
            ...
        }

    Returns one dict (component_info) keyed by either:
      – item_code (for the motor), or
      – item_name (for syringes and valve heads),
    each mapping to { item_code, uom }.
    """
    # 1) Collect all distinct valve-head names from the finished_items:
    valve_names = set()
    for itm in finished_items:
        vh = extract_valve_head_name(itm["item_name"])
        if vh:
            valve_names.add(vh)

    # 2) All syringe item_names we need:
    syringe_names = list(SYRINGE_MAP.values())
    component_info = {}

    # ── 3a) Motor: fetch its UOM in one shot
    motor_uom = frappe.get_value("Item", MOTOR_ITEM_CODE, "stock_uom")
    if not motor_uom:
        frappe.throw(
            _("Cannot find motor Item {0} or its stock_uom is empty").format(MOTOR_ITEM_CODE)
        )
    component_info[MOTOR_ITEM_CODE] = {
        "item_code": MOTOR_ITEM_CODE,
        "uom": motor_uom
    }

    # ── 3b) Syringes: bulk‐fetch items where item_name IN syringe_names
    if syringe_names:
        syringe_codes = [f"70{key}000" for key in SYRINGE_MAP.keys()]
        syr_docs = frappe.get_all(
            "Item",
            filters=[["item_code", "in", syringe_codes]],
            fields=["item_name", "item_code", "stock_uom"]
        )
        for doc in syr_docs:
            component_info[doc["item_name"]] = {
                "item_code": doc["item_code"],
                "uom": doc["stock_uom"]
            }

    # ── 3c) Valve heads: bulk‐fetch items where item_name IN valve_names
    
    for raw_name in valve_names:
        name = raw_name.strip()  # strip any accidental spaces
        # get_all always returns a list of dicts (even if there's at most one match)
        vh_docs = frappe.get_all(
            "Item",
            filters=[["reference_code", "in", name]],       # or "item_name", depending on your field
            fields=["item_name", "item_code", "stock_uom"],
            limit_page_length=1                      # optional, ensures at most one record
        )

        if vh_docs:
            # vh_docs is a list like: [{"item_name": "...", "item_code": "...", "stock_uom": "..."}]
            doc = vh_docs[0]   # take the first (and only) dict
            component_info[doc["item_name"]] = {
                "item_code": doc["item_code"],
                "uom":       doc["stock_uom"]
            }
        else:
            # If you want to catch exactly which names failed:
            print("error")
    return component_info

# ─────────────────────────────────────────────────────────────────────────────
# 4) CREATE A SINGLE BOM FOR ONE FINISHED ITEM
# ─────────────────────────────────────────────────────────────────────────────

def create_bom_for_finished_item(finished, component_info, company):
    """
    Given:
      finished = { item_code, item_name, stock_uom }  – one finished‐good item
      component_info = { … } (from fetch_all_component_info)
      company = default company name

    This function will:
      - Open (get_or_create) a Log Entry for this finished item
      - Check if a BOM already exists for `finished.item_code`. If yes, log & skip.
      - Parse the syringe from 3rd digit of finished.item_code, look up component_info.
      - Parse the valve-head name from finished.item_name, look up component_info.
      - Always use motor = component_info[MOTOR_ITEM_CODE].
      - Build a new BOM with exactly those three components (qty=1).
      - Insert() and commit() the BOM, then log success. On exception, log error.
    """

    # Fetch the full Item doc so we can pass it to _get_or_create_log():
    finished_doc = frappe.get_doc("Item", finished["item_code"])
    log_id = _get_or_create_log(finished_doc)

    # 1) Check for existing BOM
    existing = frappe.get_all(
        "BOM",
        filters={"item": finished["item_code"]},
        fields=["name"],
        limit_page_length=1
    )
    if existing:
        update_log_entry(
            log_id,
            f"BOM already exists for finished item {finished['item_code']}; skipping."
        )
        return

    # 2) Determine syringe:
    third_digit = finished["item_code"][2]
    syringe_name = SYRINGE_MAP.get(third_digit)
    if not syringe_name:
        update_log_entry(
            log_id,
            f"Invalid third digit '{third_digit}' for item {finished['item_code']}; no syringe mapping. Skipping."
        )
        return

    syringe_info = component_info.get(syringe_name)
    if not syringe_info:
        update_log_entry(
            log_id,
            f"Syringe item '{syringe_name}' not found in component_info. Skipping."
        )
        return

    # 3) Determine valve-head:
    vh_name = extract_valve_head_name(finished["item_name"])
    if not vh_name:
        update_log_entry(
            log_id,
            f"No '/…/' pattern found in item_name '{finished['item_name']}'. Cannot extract valve-head. Skipping."
        )
        return

    valve_info = component_info.get(vh_name)
    if not valve_info:
        update_log_entry(
            log_id,
            f"Valve-head item '{vh_name}' not found in component_info. Skipping."
        )
        return

    # 4) Motor (always present):
    motor_info = component_info.get(MOTOR_ITEM_CODE)
    if not motor_info:
        update_log_entry(
            log_id,
            f"Motor item {MOTOR_ITEM_CODE} missing from component_info. Skipping."
        )
        return
    print(syringe_info, motor_info, valve_info)
    input()
    # 5) Now build and insert the BOM
    try:
        bom = frappe.new_doc("BOM")
        bom.item = finished["item_code"]
        bom.item_name = finished["item_name"]
        bom.uom = finished["stock_uom"]         # UOM of the finished good
        bom.quantity = 1                         # Always 1 finished good
        bom.company = company
        bom.is_active = 1

        # Append motor, syringe, valve-head
        for comp in (motor_info, syringe_info, valve_info):
            print(comp)
        ...
        update_log_entry(
            log_id,
            f"Successfully created BOM for {finished['item_code']} with components: "
            f"{motor_info['item_code']}, {syringe_info['item_code']}, {valve_info['item_code']}."
        )

    except Exception as e:
        update_log_entry(
            log_id,
            f"Error while creating BOM for {finished['item_code']}: {frappe.utils.get_traceback()}"
        )


# ─────────────────────────────────────────────────────────────────────────────
# 5) MAIN ORCHESTRATOR: ITERATE & CREATE ALL NECESSARY BOMS
# ─────────────────────────────────────────────────────────────────────────────

def create_boms_for_all_49xxxx():
    """
    Orchestrator function. Call this to:
      • Fetch all finished items (codes “49xxxx”)
      • Bulk‐fetch all component info
      • Loop over each finished item and create its BOM if needed
    """
    # 1) Get all finished items that match “49[0-9][0-9][0-9][0-9]” (6 chars)
    finished_items = get_finished_items()


    if not finished_items:
        frappe.msgprint(_("No items with code starting '49' and length 6 were found."))
        return

    # 2) Bulk‐fetch component info (motor, syringes, valve heads)
    component_info = fetch_all_component_info(finished_items)

    # 3) Default company
    company = frappe.defaults.get_user_default("Company")
    if not company:
        frappe.throw(_("Please set a default Company for your user before running this script."))

    # 4) Loop and create BOMs one by one
    for finished in finished_items:
        create_bom_for_finished_item(finished, component_info, company)

    frappe.msgprint(_("Finished processing all ‘49xxxx’ items."))


# ─────────────────────────────────────────────────────────────────────────────
# USAGE:
# ─────────────────────────────────────────────────────────────────────────────
# To run from bench console or as a method:
#   from your_app.bom_utils import create_boms_for_all_49xxxx
#   create_boms_for_all_49xxxx()
#
# You can also hook create_boms_for_all_49xxxx() into a Server Script or a Scheduler Job.
# ─────────────────────────────────────────────────────────────────────────────
