...
# Country name to 2-letter country code mapping
country_mapping = {
    'United States': 'US',
    'Canada': 'CA',
    'Afghanistan': 'AF',
...
def get_HS_code(item_code, shipping_address_name):
    """
    Fetch the HS Code (Harmonized System Code) for a given item code from the Item doctype.

    Args:
        item_code (str): The item code for which to fetch the HS Code.
    Returns:
        str: The HS Code associated with the item code, or None if not found.
    """
    ...
    hs_type = ""
    if item.item_name.startswith("P1") :
        hs_type = "SPM et LSPone"
    elif item.item_name.startswith("P2") or item.item_code.startswith("521") :
        hs_type = "RVM"
    elif item.item_group == "Valve Head":
            hs_type = "Valve Head" 
    elif item.item_code == "C100":
            hs_type = "C100, Power Supply"
    elif item.item_code == "C101":
        hs_type = "C101, USB Cable"
    elif item.item_code == "C102":
        hs_type = "C102, DB9 Adaptor"
    elif item.item_code.startswith("C20"):
        hs_type = "Power Cord"
    elif item.item_name.startswith("Quick Start Kit"):
        hs_type = "Quick Start Kit"
    elif item.item_code == "T100":
        hs_type = "T100, Syringe Change Tool"
    elif item.item_group == "Syringe":
        hs_type = "Syringe"
    else:
        return None
